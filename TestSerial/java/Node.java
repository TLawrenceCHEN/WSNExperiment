...
class Node {
    ...
    /* Data received containing NREADINGS samples from messageId * NREADINGS 
       onwards */
    void update(int messageId, int dt) {//int temperature, int humidity, int light
	    int start = messageId * Constants.NREADINGS;
	    setEnd(start, start + Constants.NREADINGS);
	    //data[start - dataStart] = temperature;
	    //data[start - dataStart + 1] = humidity;
	    data[start - dataStart] = dt;
    }
