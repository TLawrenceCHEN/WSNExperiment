...
class Data {
    ...
    synchronized void update(int nodeId, int messageId, int dt) {//int temperature, int humidity, int light
	    ...
	    if (parent.flag[nodeId] == false) {
	    	parent.flag[nodeId] = true;
			parent.newNode(nodeId);
	    }
    	if (node == null){
			nodes[nodeId] = node = new Node(nodeId);
		}
	    node.update(messageId, dt);//temperature, humidity, light);
	}
