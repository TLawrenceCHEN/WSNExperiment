...
class Graph extends JPanel
{
    ...
    private class SingleGraph {
		...
		SingleGraph(Graphics g, int id) {
			datasize = parent.parent.datasize;
		    nodeId = id;
		    lastsx = new int [datasize];
		    lastsy = new int [datasize];
		...
		void nextPoint(Graphics g, int gx, int sx) {
			for (int i = 0; i < datasize; i++){
				int gy = parent.parent.data[i].getData(nodeId, gx);
				int sy = -1;
				if (gy >= 0) { // Ignore missing values
					...
					if (lastsy[i] >= 0 && sy >= 0) {
						g.drawLine(lastsx[i], lastsy[i], sx, sy);
					}
				}
				lastsx[i] = sx;
				lastsy[i] = sy;
    ...
    void newData() {
    	for (int i = 0; i < parent.parent.datasize; i++){
    		int max = parent.parent.data[i].maxX();
			if (max > gx1 || max < gx0) {
				recenter(max);
			}
			repaint();
    ...
    void setScale(int newScale) {
		...
		int max = 0;
		for (int i = 0; i < parent.parent.datasize; i++){
			max = (max < parent.parent.data[i].maxX()) ? parent.parent.data[i].maxX():max;
		}
		recenter(max);
		repaint();
    }
